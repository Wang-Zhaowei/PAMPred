...
def pso(func, lb, ub, seed=0, ieqcons=[], f_ieqcons=None, args=(), kwargs={}, swarmsize=100, maxiter=100, minstep=1e-8, minfunc=1e-8):
    ...
    while it<=maxiter:
        ...
        for i in range(S):
            ...
            phip = phip_init + i * (phip_end - phip_init)/maxiter
            phig = phig_init + i * (phig_end - phig_init)/maxiter
...
def extract_weight(opt_weights):
    weights = []
    for i in range(len(opt_weights)):
        weights.append(opt_weights[i]/sum(opt_weights))
    return weights
